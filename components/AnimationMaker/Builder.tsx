  ...
  return (
    <>
      ...
      <div className={`h-full transition-all duration-300 ${store.isFullScreen ? 'fixed inset-0 z-50 bg-slate-950 flex flex-col' : 'grid grid-cols-1 lg:grid-cols-12 gap-8'}`}>
        <div className={`flex flex-col space-y-6 overflow-y-auto max-h-[40vh] lg:max-h-[calc(100vh-140px)] custom-scrollbar pr-2 transition-all duration-300 ${store.isFullScreen ? 'hidden' : 'lg:col-span-4'}`}>
        ...
        <div className={`relative flex flex-col gap-4 transition-all duration-300 ${store.isFullScreen ? 'flex-1 w-full h-full p-4' : 'lg:col-span-8 h-full min-h-[50vh] lg:min-h-[600px]'}`}>
           <BuilderViewport 
              iframeRef={iframeRef}
